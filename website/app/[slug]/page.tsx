...
const Page = async ({ params }: { params: Promise<{ slug: string }> }) => {
	...
	return (
		<>
			...
			<section>
				<div className='w-full border border-x-0 border-b-0 border-t-neutral-800 pt-6 px-6'>
			...
			<section>
				<div className='flex w-full flex-col px-6 pb-6'>
					{Object.entries(CODE).map(([key, item]: [string, any]) => {
						const codeItem = typeof item === 'function' ? item(slug ?? null, speed ?? null, keyframes ?? null) : item;
						return (
							<div className="mt-6" key={key}>
								<CodeBlock lang={codeItem.lang ?? 'tsx'} title={codeItem.title} code={codeItem.code} />
							</div>
						);
					})}
				</div>
